...
export function TaskManager({ userId, projects, tasks, setTasks, teams, activityLog, setActivityLog }) {
  ...
  const saveTask = (data) => {
    if (editingTask) {
      const oldMemberId = editingTask.assignedMemberId
      const newMemberId = data.assignedMemberId ? parseInt(data.assignedMemberId) : null
      const oldMemberName = getMemberName(oldMemberId)
      const newMemberName = getMemberName(newMemberId)
      ...
      // Log assignment change if member was reassigned
      if (oldMemberId !== newMemberId && newMemberId) {
        const newLog = {
          id: Date.now() + Math.random(),
          time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
          message: `Task "${data.title}" reassigned from ${oldMemberName} to ${newMemberName}`,
        }
        const updatedLogs = [newLog, ...activityLog]
        const trimmedLogs = updatedLogs.slice(0, 10)
        setActivityLog(trimmedLogs)
        localStorage.setItem(`log_${userId}`, JSON.stringify(trimmedLogs))
      }
      ...
      if (newTask.assignedMemberId) {
        const memberName = getMemberName(newTask.assignedMemberId)
        const newLog = {
          id: Date.now() + Math.random(),
          time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
          message: `Task "${newTask.title}" assigned to ${memberName}`,
        }
        const updatedLogs = [newLog, ...activityLog]
        const trimmedLogs = updatedLogs.slice(0, 10)
        setActivityLog(trimmedLogs)
  ...
  for (const team of teams) {
    // 2. Try to find the member in the current team's members array
    const member = team.members?.find((m) => m.id === memberId);
    
    // 3. If the member is found, return their name immediately and exit the function
    if (member) {
      return member.name;
    }
  }
  ...
  //    return the default 'Unassigned' string.
  return "Unassigned";
};
